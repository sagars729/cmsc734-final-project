...
const KeyPointsList = (props: any) => {
  ...
  const deletePoint = (time: string, index: number) => {
    ...
    var len = newData.find((val) => {
      return val.time === time;
    })?.points.length;

    if (len === 0) {
      var toRemove = newData.findIndex((val) => {
        return val.time === time;
      });
              ...
              <div className="card card-margin">
                ...
                {item.points.map((point: any, pointIdx: number) => (
                  <div
                    ...
                    key={`item_${item}_pointIndex_${pointIdx}`}
                  >
                    <div className="widget-49">
                      ...
                      <ol className="widget-49-meeting-points">
                        <div className="form-outline">
                          <textarea
                            ...
                            value={point.analysis_yielded}
                            onChange={(e) => {
                              let newData = [...props.data];
                              newData[idx].points[pointIdx].analysis_yielded =
                                e.target.value;
                              props.setData(newData);
                      ...
                      <div className="widget-49-meeting-action">
                        <button
                          onClick={(e) => deletePoint(item.time, pointIdx)}
