...
const SectionBuilder = (props: SectionBuilderProps) => {
  ...
  const addSection = () => {
    if (brushStartDate && brushEndDate) {
      props.setSections(props.sections.concat([{
  ...
  const popSection = () => {
    if (props.sections.length > 0) {
      props.setSections(props.sections.slice(0, props.sections.length - 1))
    }
  }
    ...
    <div id="sectionBuilder" style={{margin:"5%"}}>
      ...
      <Box display="flex" alignItems="center" justifyContent="center" flexDirection="column">
        ...
        {brushStartDate && brushEndDate ? (
          <p> 
            Selected Area: {brushStartDate.toLocaleDateString('en-US')}
            - {brushEndDate.toLocaleDateString('en-US')}
          </p>
        ) : <></>}
        <ButtonGroup variant="contained" aria-label="outlined primary button group">
          <Button color='success' onClick={addSection}>Add Section</Button>
          <Button color='warning' onClick={popSection}>Pop Section</Button>
        </ButtonGroup>
      </Box>
      <Box sx={builderSectionStyle}>
        <p>
          Modify the section headers below. The annotations that you added
          previously will be displayed as parragraphs under these new headers.
          Select a chart area and click the "Add Section" button to create a
          new section. Click "Pop Section" to remove the last added section.
        </p>
        ...
        <Grid container spacing={2} sx={{margin: "16px"}}>
          ...
          {props.sections.map((section, i) => (
            <>
              <Grid xs={3}> {section.startDate.toLocaleDateString('en-US')} </Grid>
              <Grid xs={3}> {section.endDate.toLocaleDateString('en-US')} </Grid>
              <Grid xs={6}>
                <CustomTextField
